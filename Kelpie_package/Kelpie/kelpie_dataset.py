...
class KelpieDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.kelpie_entity_id = dataset.num_entities
        ...
        self.kelpie_train_samples_copy = copy.deepcopy(
    ...
    def add_training_samples(self, samples_to_add: numpy.array):
        ...
        for (cur_head, cur_rel, cur_tail) in kelpie_samples_to_add:
            ...
            self.to_filter[(cur_tail, cur_rel +
                            self.num_direct_relations)].append(cur_head)
            self.train_to_filter[(cur_head, cur_rel)].append(cur_tail)
            self.train_to_filter[(cur_tail, cur_rel +
                                  self.num_direct_relations)].append(cur_head)

            self.last_added_kelpie_samples.append(
                (cur_head, cur_rel, cur_tail))
            self.last_filter_additions[(cur_head, cur_rel)].append(cur_tail)
            self.last_filter_additions[(
                cur_tail, cur_rel + self.num_direct_relations)].append(cur_head)
    ...
    def undo_last_training_samples_addition(self):
        ...
        self.kelpie_train_samples = copy.deepcopy(
            self.kelpie_train_samples_copy)
    ...
    def remove_training_samples(self, samples_to_remove: numpy.array):
        ...
        for (cur_head, cur_rel, cur_tail) in kelpie_train_samples_to_remove:
            ...
            self.to_filter[(cur_tail, cur_rel +
                            self.num_direct_relations)].remove(cur_head)
            ...
            self.last_removed_kelpie_samples.append(
                (cur_head, cur_rel, cur_tail))
    ...
    def as_kelpie_sample(self, original_sample):
        if not self.original_entity_id in original_sample:
            raise Exception("Could not find the original entity " + str(
                self.original_entity_id) + " in the passed sample " + str(original_sample))
