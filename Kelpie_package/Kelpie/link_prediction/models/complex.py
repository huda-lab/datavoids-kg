...
from Kelpie.link_prediction.models.model import Model, KelpieModel, DIMENSION, INIT_SCALE
...
class ComplEx(Model):
    ...
    def __init__(self,
        ...
        self.real_dimension = hyperparameters[DIMENSION]
        ...
        if init_random:
            self.entity_embeddings = Parameter(torch.rand(
                self.num_entities, self.dimension).cuda(), requires_grad=True)
    ...
    def score_embeddings(self,
        ...
        lhs = head_embeddings[:,
                              :self.real_dimension], head_embeddings[:, self.real_dimension:]
        # split the relation embedding into real and imaginary components
        rel = rel_embeddings[:,
                             :self.real_dimension], rel_embeddings[:, self.real_dimension:]
        # split the tail embedding into real and imaginary components
        rhs = tail_embeddings[:,
                              :self.real_dimension], tail_embeddings[:, self.real_dimension:]
    ...
    def forward(self, samples: np.array):
        ...
        to_score = to_score[:, :self.real_dimension], to_score[:,
                                                               self.real_dimension:]
    ...
    def _get_queries(self, samples: np.array):
        ...
        relation_embeddings = relation_embeddings[:,
                                                  :self.real_dimension], relation_embeddings[:, self.real_dimension:]
    ...
    def predict_samples(self, samples: np.array) -> Tuple[Any, Any, Any]:
        ...
        direct_scores, tail_ranks, tail_predictions = self.predict_tails(
            direct_samples)
    ...
    def predict_tails(self, samples: np.array) -> Tuple[Any, Any, Any]:
        ...
        with torch.no_grad():
            ...
            # ranks was initialized with ONES
...
class KelpieComplEx(KelpieModel, ComplEx):
    def __init__(
        ...
        self.kelpie_entity_embedding = Parameter(
            init_tensor.cuda(), requires_grad=True)
    ...
    def predict_samples(self,
        ...
        scores, ranks, predictions = ComplEx.predict_samples(
            self, direct_samples)
        ...
        for i in range(len(direct_samples)):
            ...
            forbidden_indices = np.where(
                head_predictions == forbidden_entity_id)[0]
            ...
            forbidden_indices = np.where(
                tail_predictions == forbidden_entity_id)[0]
            if len(forbidden_indices) > 0:
                index = forbidden_indices[0]
