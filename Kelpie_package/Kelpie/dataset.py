...
class Dataset:
    ...
    def __init__(self,
        ...
        if load:
            user_provided_specific_paths = train_path and test_path and valid_path
            if not os.path.isdir(self.home) and not user_provided_specific_paths:
            ...
            # also count inverse relations
            self.num_relations = 2*len(self.relations)
            ...
            for relation_id in range(self.num_direct_relations):
                ...
                inverse_relation_name = "INVERSE_" + \
                    self.relation_id_2_name[relation_id]
            ...
            for i in range(all_samples.shape[0]):
                ...
                self.to_filter[(tail_id, relation_id +
                                self.num_direct_relations)].append(head_id)
                ...
                if i < len(self.train_samples):
                    self.train_to_filter[(
                        head_id, relation_id)].append(tail_id)
                    self.train_to_filter[(
                        tail_id, relation_id + self.num_direct_relations)].append(head_id)
    ...
    def _read_triples(self, triples_path: str, separator="\t"):
        ...
        with open(triples_path, "r") as triples_file:
            ...
            for line in lines:
                # this is required for some YAGO3-10 lines
    ...
    def _compute_relation_2_type(self):
        ...
        for relation in relation_2_heads_nums:
            average_heads_per_tail = numpy.average(
                relation_2_heads_nums[relation])
            average_tails_per_head = numpy.average(
                relation_2_tails_nums[relation])
    ...
    def add_training_samples(self, samples_to_add: numpy.array):
        ...
        for (head, rel, tail) in samples_to_add:
            ...
            self.to_filter[(tail, rel + self.num_direct_relations)
                           ].append(head)
            self.train_to_filter[(head, rel)].append(tail)
            self.train_to_filter[(
                tail, rel + self.num_direct_relations)].append(head)
    ...
    def remove_training_samples(self, samples_to_remove: numpy.array):
        ...
        for sample_to_remove in samples_to_remove:
            ...
            if (head, rel, tail) in self.train_samples_set:
                ...
                self.to_filter[(
                    tail, rel + self.num_direct_relations)].remove(head)
                self.train_samples_set.remove((head, rel, tail))
                output.append(True)
            else:
                output.append(False)

        self.train_samples = numpy.delete(
            self.train_samples, indices_to_remove, axis=0)
        return output
    ...
    def remove_training_sample(self, sample_to_remove: numpy.array):
        ...
        if (head, rel, tail) in self.train_samples_set:
            index = numpy.where(
                numpy.all(self.train_samples == sample_to_remove, axis=1))
            self.train_samples = numpy.delete(
                self.train_samples, index[0], axis=0)
    ...
    def save_training_set(self, fn):
        with open(fn, "w") as outFile:
            for sample in self.train_samples_set:
                outFile.write('\t'.join(self.sample_to_fact(sample)) + '\n')
