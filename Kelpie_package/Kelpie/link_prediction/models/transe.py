...
from Kelpie.link_prediction.models.model import Model, DIMENSION, KelpieModel
...
class TransE(Model):
    ...
    def __init__(self,
        ...
        if init_random:
            ...
            self.relation_embeddings = Parameter(torch.rand(
                self.num_relations, self.dimension).cuda(), requires_grad=True)
    ...
    def score(self, samples: np.array) -> np.array:
        ...
        rel_embeddings = self.relation_embeddings[samples[:, 1]]
    ...
    def all_scores(self, samples: np.array):
        ...
        all_tail_embeddings = self.entity_embeddings
        ...
        all_scores = (translation_outputs.unsqueeze(
            0) - all_tail_embeddings.unsqueeze(1)).norm(p=1, dim=2)
...
class KelpieTransE(KelpieModel, TransE):
    ...
    def predict_samples(self,
        ...
        scores, ranks, predictions = TransE.predict_samples(
            self, direct_samples)
