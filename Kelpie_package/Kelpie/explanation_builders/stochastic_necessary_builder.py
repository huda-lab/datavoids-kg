...
from Kelpie.explanation_builders.explanation_builder import NecessaryExplanationBuilder
import numpy
...
class StochasticNecessaryExplanationBuilder(NecessaryExplanationBuilder):
    ...
    def build_explanations(self, samples_to_remove: list, top_k: int = 10):
        ...
        if best_rule_relevance > self.xsi:
            return all_rules_with_relevance[:top_k]
        ...
        print(
            "len in builder:",
            len(
                sorted(all_rules_with_relevance, key=lambda x: x[1], reverse=True)[
                    :top_k
                ]
            ),
        )
